...
def build_pedigree(
    ...
    def node(a: Optional[Animal], depth: int) -> Dict[str, Any]:
        ...
        result: Dict[str, Any] = {
            "id": a.id,
            "name": a.name,
            "species": getattr(a.species, "value", a.species) if hasattr(a, "species") else None,
        ...
        result["sire"] = node(sire, depth - 1) if sire else ({"name": getattr(a, "sire_name", None)} if getattr(a, "sire_name", None) else {})
        result["dam"] = node(dam, depth - 1) if dam else ({"name": getattr(a, "dam_name", None)} if getattr(a, "dam_name", None) else {})
